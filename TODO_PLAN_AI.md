...
11. [DONE] Напоминания / уведомления

- Настроен in-app reminder flow: можно выбрать время в Habit detail/edit, Dashboard показывает reminder banner с Mark done/Dismiss, и клиент следит за синхронизированным `reminderTime`.
- Push-уведомления сохраняются как future work, текущая реализация держит пользователей в курсе через видимые напоминания в UI.

12. [DONE] Streak protection / "freeze days"
...
13. [DONE] Порядок привычек (drag & drop)

- Добавлено поле `sortOrder`, поддерживается синхронизация с сервером, и Dashboard умеет drag-and-drop через grip handle на карточках; порядок закрепляется после drop и сохраняется между сессиями.

14. [DONE] Категории / группировка

- Tags есть, но нет группировки по ним на Dashboard. Добавить секции или фильтр-табы по тегам.

15. [DONE] Экспорт данных

- CSV/JSON экспорт привычек и completions. Важно для доверия пользователей.

16. [DONE] Onboarding / пустое состояние
...
17. [DONE] Undo для чекинов и удалений
...
├─────────┼───────┼─────────────────────────────────────────┼────────┼────────────────────────────────────────────────────────┤
│ 9       │ P2.13 │ [DONE] Drag & drop порядок (sortOrder synced) │ M      │ Среднее — UX                                           │
├─────────┼───────┼─────────────────────────────────────────┼────────┼────────────────────────────────────────────────────────┤
│ 10      │ P2.12 │ Freeze days                             │ M      │ Среднее — мотивация                                    │
├─────────┼───────┼─────────────────────────────────────────┼────────┼────────────────────────────────────────────────────────┤
│ 11      │ P2.11 │ [DONE] Reminder scheduling (in-app banner)  │ L      │ Высокое — engagement                                   │
├─────────┼───────┼─────────────────────────────────────────┼────────┼────────────────────────────────────────────────────────┤
│ 12      │ P2.15 │ [DONE] Экспорт данных                    │ S      │ Среднее — доверие                                      │
├─────────┼───────┼─────────────────────────────────────────┼────────┼────────────────────────────────────────────────────────┤
