...
- Copy `.env.example` to `.env` and tune the following keys:
  ...
  - `VITE_DEFAULT_USER_ID` is the user ID embedded into IndexedDB and outgoing requests
...
### API
   ...
   2. Copy `server/.env.example` to `server/.env` and update each value to match your database and secrets:
      ...
      - `ALLOW_LEGACY_X_USER` gates the `x-user-id` fallback in dev and QA
      - `DEFAULT_USER_ID` keeps the sync controller and client aligned when auth is not enforced
