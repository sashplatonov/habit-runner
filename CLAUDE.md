...
The legacy NestJS + Prisma reference backend has been removed from the workspace. The active backend is now `apps/backend/` (Quarkus + Flyway + PostgreSQL). If you need archived Prisma artifacts, check `archive/`.
...
### Server (`apps/backend/src/main/java/com/habittracker/`)
...
- **Database schema**: Flyway SQL migrations in `apps/backend/src/main/resources/db/migration/`.
...
| `apps/backend/.env` | `DATABASE_URL`, `DB_USER`, `DB_PASSWORD` | Local (non-Docker) PostgreSQL connection settings |
| `apps/backend/.env` | `AUTH_SECRET`, `ACCESS_TOKEN_TTL_SECONDS`, `REFRESH_TOKEN_EXPIRES_DAYS` | Local JWT settings |
| `apps/backend/.env` | `GOOGLE_OAUTH_CLIENT_ID/SECRET` | Local Google OAuth credentials |
| `apps/backend/.env` | `API_PUBLIC_URL`, `OAUTH_DEFAULT_RETURN_TO`, `CORS_ORIGINS` | Local OAuth and CORS settings |
