...
export function HabitDetail() {
  ...
  const handleIncrementCompletion = useCallback(async () => {
    ...
    await setCompletionCount(habitId, today, nextCount);
    push({
      message: `Progress ${nextCount}/${target}: ${habit.name}`,
      actionLabel: 'Undo',
      onUndo: async () => {
        await setCompletionCount(habitId, today, previousCount);
      }
    });
  }, [habit, habitId, push, setCompletionCount, today]);
  ...
  const completedToday = (habit.completions[today] ?? 0) >= Math.max(1, habit.dailyTarget ?? 1);
  const todayCompletionCount = habit.completions[today] ?? 0;
