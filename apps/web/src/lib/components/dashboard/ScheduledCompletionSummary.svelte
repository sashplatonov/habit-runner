...
<Surface as="section" padding="none" class="relative overflow-hidden" ariaLive="polite">
  ...
  <div data-layout="desktop" class="desktop-summary p-4 sm:p-5">
    ...
    <div class="mt-5 border-t border-border/60 pt-4">
      ...
      <div class="grid grid-cols-[repeat(30,minmax(0,1fr))] gap-1" role="img" aria-label="30-day scheduled completion heatmap">
    ...
    <div class="mt-4 border-t border-border/60 pt-4">
      ...
      <div class="mt-2 grid min-w-0 gap-1" style={`grid-template-columns: repeat(${Math.max(summary.today.segments.length, 1)}, minmax(0, 1fr));`} role="img" aria-label={todayBarLabel()}>
  ...
  <div data-layout="mobile" class="mobile-summary gap-1.5 p-2.5">
    <div class="grid min-w-0 grid-cols-[70px_minmax(0,1fr)_42px] items-center gap-1.5">
      <strong class="text-[11px] font-extrabold text-foreground">Completion</strong>
      <span class="truncate text-[9px] text-muted">{dateLabel} · scheduled only</span>
      <strong class="text-right text-[10px] font-extrabold tabular-nums text-foreground">{scoreLabel(summary.today.percentage)}</strong>
    </div>
    <div class="grid min-w-0 grid-cols-[70px_minmax(0,1fr)_42px] items-center gap-1.5">
      <span class="text-[9px] text-muted">30 days</span>
      <div class="grid min-w-0 grid-cols-[repeat(30,minmax(0,1fr))] gap-px" role="img" aria-label="30-day scheduled completion heatmap">
        {#each summary.days as day, index (day.calendarDate + '-' + index)}
          <span class={`h-1.5 min-w-0 rounded-[2px] ${cellClass(day)}`} aria-label={dayLabel(day)}></span>
        {/each}
      </div>
      <span class="text-right text-[10px] font-extrabold tabular-nums text-foreground">{summary.perfectDays}✓</span>
    </div>
    <div class="grid min-w-0 grid-cols-[70px_minmax(0,1fr)_42px] items-center gap-1.5">
      <span class="text-[9px] text-muted">Today {summary.today.completed}/{summary.today.required}</span>
      <div class="grid min-w-0 gap-px" style={`grid-template-columns: repeat(${Math.max(summary.today.segments.length, 1)}, minmax(0, 1fr));`} role="img" aria-label={todayBarLabel()}>
        {#each summary.today.segments as segment, index (segment.habitId + '-' + index)}
