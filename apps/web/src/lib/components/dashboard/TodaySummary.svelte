...
<Surface as="section" padding="sm" class="relative overflow-hidden" ariaLive="polite">
  ...
  <div class="relative space-y-3">
    <div class="flex flex-wrap items-start justify-between gap-x-4 gap-y-2">
      <div class="min-w-0">
        ...
        <h1 class="mt-1 text-pretty text-lg font-semibold tracking-tight text-foreground sm:text-xl">
          {summary.headline}
        </h1>
      </div>
    ...
    <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
      <p class="min-w-0 break-words text-xs leading-5 text-muted sm:text-sm">
        {summary.message}
      </p>
      ...
      {#if summary.nextActionLabel && onPrimaryAction}
        <button
          type="button"
          class="inline-flex min-h-11 shrink-0 items-center justify-center rounded-full border border-accent/25 bg-accent/12 px-4 py-2 text-sm font-semibold text-accent transition-colors hover:border-accent/40 hover:bg-accent/16"
          onclick={() => {
