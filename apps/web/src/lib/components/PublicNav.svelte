  ...
  <div class="mx-auto flex w-full {maxWidth} items-center gap-4 px-4 py-4 sm:px-6">
    ...
    <nav class="hidden flex-1 items-center gap-1 sm:flex" aria-label="Main navigation">
        <a
          ...
          class="rounded-full px-3 py-2 text-xs font-medium transition-[background-color,color,box-shadow] {page.url.pathname === featuresHref ? 'bg-bg-card text-foreground shadow-[0_12px_24px_rgba(15,23,42,0.12)]' : 'text-muted hover:bg-bg-card hover:text-foreground'}"
        >Features</a>
        <a
          href={resolve<'/blog'>('/blog', {})}
          aria-current={page.url.pathname === blogHref ? 'page' : undefined}
          class="rounded-full px-3 py-2 text-xs font-medium transition-[background-color,color,box-shadow] {page.url.pathname === blogHref ? 'bg-bg-card text-foreground shadow-[0_12px_24px_rgba(15,23,42,0.12)]' : 'text-muted hover:bg-bg-card hover:text-foreground'}"
        >Blog</a>
        <a
          href={resolve<'/about'>('/about', {})}
          aria-current={page.url.pathname === aboutHref ? 'page' : undefined}
          class="rounded-full px-3 py-2 text-xs font-medium transition-[background-color,color,box-shadow] {page.url.pathname === aboutHref ? 'bg-bg-card text-foreground shadow-[0_12px_24px_rgba(15,23,42,0.12)]' : 'text-muted hover:bg-bg-card hover:text-foreground'}"
        >About</a>
    </nav>
    ...
    <div class="ml-auto flex items-center gap-2 sm:gap-3">
      ...
      {:else}
        <button
          ...
          class="rounded-full border border-progress/20 bg-bg-card px-4 py-2 text-xs font-semibold text-foreground shadow-[0_12px_24px_rgba(15,23,42,0.08)] transition-[transform,border-color,color] hover:-translate-y-0.5 hover:border-progress/30 hover:text-progress"
        >
          Get Started
        </button>
      {/if}
