      ...
      <nav class="grid gap-5 text-xs text-muted sm:grid-cols-3" aria-label="Footer navigation">
        <div>
          ...
          <div class="flex flex-col gap-2.5">
            ...
            <a href={resolve<'/features'>('/features', {})} class="transition-colors hover:text-foreground">Features</a>
            <a href={resolve<'/blog'>('/blog', {})} class="transition-colors hover:text-foreground">Blog</a>
            <a href={resolve<'/about'>('/about', {})} class="transition-colors hover:text-foreground">About</a>
          </div>
        </div>
        <div>
          <p class="mb-3 text-[10px] uppercase tracking-[0.24em] text-muted">Guides</p>
          <div class="flex flex-col gap-2.5">
            <a href={resolve<'/habit-tracker'>('/habit-tracker', {})} class="transition-colors hover:text-foreground">Habit Tracker</a>
            <a href={resolve<'/streak-tracker'>('/streak-tracker', {})} class="transition-colors hover:text-foreground">Streak Tracker</a>
            <a href={resolve<'/daily-routine-planner'>('/daily-routine-planner', {})} class="transition-colors hover:text-foreground">Daily Routine Planner</a>
            <a href={resolve<'/vs/[slug]'>('/vs/[slug]', { slug: 'habitica' })} class="transition-colors hover:text-foreground">vs Habitica</a>
          </div>
        </div>
        <div>
          <p class="mb-3 text-[10px] uppercase tracking-[0.24em] text-muted">Legal</p>
          <div class="flex flex-col gap-2.5">
            <a href={resolve<'/privacy-policy'>('/privacy-policy', {})} class="transition-colors hover:text-foreground">Privacy</a>
          </div>
        </div>
      </nav>
    </section>
